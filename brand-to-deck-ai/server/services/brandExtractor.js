...
async function extractBrandConfiguration(pdfData, filePath) {
  try {
    ...
    // Extraemos solo el bloque JSON usando una expresión regular
    const jsonMatch = responseText.match(/```json\n([\s\S]*?)\n```/);
    if (!jsonMatch || !jsonMatch[1]) {
      throw new Error("No se pudo encontrar un bloque JSON válido en la respuesta de Gemini.");
    }
    const jsonString = jsonMatch[1].trim();
    console.log("Debug: JSON string limpiado:", jsonString);
    
    const extractedConfig = JSON.parse(jsonString);
